...
def generate_dataframe_labels():
    ...
    trajectory_labels = ['Design', 'Protocol', 'Length', 'Seed', 'Helicity', 'Target_Hotspot', 'Sequence', 'InterfaceResidues', 'pLDDT', 'pTM', 'i_pTM', 'pAE', 'i_pAE', 'ipSAE', 'pDockQ', 'pDockQ2', 'LIS', 'i_pLDDT', 'ss_pLDDT', 'Unrelaxed_Clashes',
    ...
    core_labels = ['pLDDT', 'pTM', 'i_pTM', 'pAE', 'i_pAE', 'ipSAE', 'pDockQ', 'pDockQ2', 'LIS', 'i_pLDDT', 'ss_pLDDT', 'Unrelaxed_Clashes', 'Relaxed_Clashes', 'Binder_Energy_Score', 'Surface_Hydrophobicity',
...
def generate_directories(design_path):
    ...
    design_path_names = ["Accepted", "Accepted/Ranked", "Accepted/Animation", "Accepted/Plots", "Accepted/Pickle", "Trajectory",
                        ...
                        "Trajectory/Pickle", "MPNN", "MPNN/Binder", "MPNN/Sequences", "MPNN/Relaxed", "MPNN/Maturation", "Rejected"]
    design_paths = {}
...
def update_last_csv_row(csv_file, data_array):
    """Replace the last row of a CSV with updated data (e.g. after maturation populates placeholder columns)."""
    t0 = time.time()
    df = pd.read_csv(csv_file)
    if len(df) > 0:
        df.iloc[-1] = data_array
        df.to_csv(csv_file, index=False)
        vprint(f"[GenUtils] Updated last row of {csv_file} in {time.time()-t0:.2f}s")
    ...
    for model_num in range(1, 6):  # assumes models are numbered 1 through 5
        ...
        if model_num in statistics:
            ...
            for stat, value in model_stats.items():
                # Skip private/cached keys (e.g. _pae_matrix, _plddt_array)
