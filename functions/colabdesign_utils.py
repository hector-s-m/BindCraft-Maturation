...
from .ipsae_utils import calculate_ipsae, calculate_contact_scores
from .maturation_utils import assess_interface_residue_quality, partition_interface_residues, format_fixed_positions_for_mpnn, get_maturation_metric, log_maturation_round


def _configure_af_model(af_model, advanced_settings, helicity_value):
    """Configure AF2 model weights, contacts, and loss functions (shared between hallucination and maturation)."""
    ...
    af_model.opt["con"].update({"num": advanced_settings["intra_contact_number"],
                                 "cutoff": advanced_settings["intra_contact_distance"], "binary": False, "seqsep": 9})
    af_model.opt["i_con"].update({"num": advanced_settings["inter_contact_number"],
                                   "cutoff": advanced_settings["inter_contact_distance"], "binary": False})
...
def binder_hallucination(design_name, starting_pdb, chain, target_hotspot_residues, length, seed, helicity_value, design_models, advanced_settings, design_paths, failure_csv):
    ...
    ### Update weights, contacts, and loss functions
    ...
    if advanced_settings["design_algorithm"] == '2stage':
        ...
        if initial_plddt > 0.65:
            ...
            if advanced_settings["optimise_beta"]:
                ...
                if float(beta) > 15:
                    soft_iters = advanced_settings["soft_iterations"] + advanced_settings["optimise_beta_extra_soft"]
                    temp_iters = advanced_settings["temporary_iterations"] + advanced_settings["optimise_beta_extra_temp"]
            ...
            if temp_iters > 0:
                ...
                af_model.clear_best()
                t_soft = time.time()
                af_model.design_soft(temp_iters, e_temp=1e-2, models=design_models, num_models=1,
...
def predict_binder_complex(prediction_model, binder_sequence, mpnn_design_name, target_pdb, chain, length, trajectory_pdb, prediction_models, advanced_settings, filters, design_paths, failure_csv, seed=None, use_pyrosetta=True):
    ...
    for model_num in prediction_models:
        ...
        if not os.path.exists(complex_pdb):
            ...
            # Extract arrays for interface scoring and maturation
            pae_matrix = np.array(prediction_model.aux["pae"])
            plddt_array = np.array(prediction_model.aux["plddt"])
            target_len = prediction_model._target_len
            binder_len = prediction_model._binder_len

            # Calculate interface quality scores (ipSAE, pDockQ, pDockQ2, LIS)
            try:
                ipsae_score = calculate_ipsae(pae_matrix, target_len, binder_len)['ipSAE']
            except Exception as e:
                print(f"  Warning: ipSAE calculation failed: {e}")
                ipsae_score = None

            try:
                contact_scores = calculate_contact_scores(complex_pdb, pae_matrix, plddt_array, target_len, binder_len)
            except Exception as e:
                print(f"  Warning: contact scores calculation failed: {e}")
                contact_scores = {'pDockQ': None, 'pDockQ2': None, 'LIS': None}
            ...
            stats = {
                ...
                'ipSAE': ipsae_score,
                'pDockQ': contact_scores['pDockQ'],
                'pDockQ2': contact_scores['pDockQ2'],
                'LIS': contact_scores['LIS'],
                '_pae_matrix': pae_matrix,      # cached for maturation (not written to CSV)
                '_plddt_array': plddt_array,    # cached for maturation (not written to CSV)
                '_target_len': target_len,
                '_binder_len': binder_len,
            }
...
# run MPNN with specific residues fixed for maturation
def mpnn_gen_sequence_maturation(trajectory_pdb, binder_chain, fixed_positions_str, advanced_settings):
    """Generate MPNN sequences with maturation-selected residues fixed."""
    clear_mem()

    mpnn_model = mk_mpnn_model(backbone_noise=advanced_settings["backbone_noise"], model_name=advanced_settings["model_path"], weights=advanced_settings["mpnn_weights"])

    design_chains = 'A,' + binder_chain
    fixed_positions = fixed_positions_str.rstrip(",")
    print("Maturation MPNN fixing positions: " + fixed_positions)

    mpnn_model.prep_inputs(pdb_filename=trajectory_pdb, chain=design_chains, fix_pos=fixed_positions, rm_aa=advanced_settings["omit_AAs"])

    num_seqs = advanced_settings.get("maturation_num_seqs", 5)
    sampling_temp = advanced_settings.get("maturation_sampling_temp", 0.1)
    mpnn_sequences = mpnn_model.sample(temperature=sampling_temp, num=1, batch=num_seqs)

    return mpnn_sequences


# AF2 partial re-hallucination for maturation (backbone + sequence redesign)
def binder_maturation_hallucination(design_name, starting_pdb, chain, target_hotspot_residues, length,
    ...
    hard_iters = advanced_settings.get("maturation_hard_iterations", 5)
    greedy_iters = advanced_settings.get("maturation_greedy_iterations", 10)
    greedy_tries = max(1, math.ceil((length - len(fixed_positions)) * (advanced_settings["greedy_percentage"] / 100)))

    print(f"  Maturation hallucination: soft={soft_iters}, hard={hard_iters}, greedy={greedy_iters}, "
          f"fixed={len(fixed_positions)}/{length} positions")

    # Skip logits stage — we already have a good starting point
    # Stage 1: Softmax refinement
    if soft_iters > 0:
        af_model.design_soft(soft_iters, e_temp=1e-2, models=design_models, num_models=1,
                              sample_models=advanced_settings["sample_models"], ramp_recycles=False, save_best=True)

    # Stage 2: One-hot discretization
    if hard_iters > 0:
        af_model.clear_best()
        af_model.design_hard(hard_iters, temp=1e-2, models=design_models, num_models=1,
                              sample_models=advanced_settings["sample_models"], dropout=False,
                              ramp_recycles=False, save_best=True)

    # Stage 3: PSSM semigreedy mutations on non-fixed positions
    if greedy_iters > 0:
        af_model.design_pssm_semigreedy(soft_iters=0, hard_iters=greedy_iters, tries=greedy_tries,
                                         models=design_models, num_models=1,
                                         sample_models=advanced_settings["sample_models"],
                                         ramp_models=False, save_best=True)

    # Save maturation trajectory PDB
    af_model.save_pdb(model_pdb_path)

    return af_model, model_pdb_path
